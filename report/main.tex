...
%   4. Results from the Kaggle run's results.json are already integrated
%      (best model = ConvNeXt-Tiny); no \res{} placeholders remain.
% =====================================================================
...
% --- the best model name (highest accuracy/F1; also the saved + deployed best_model.keras) ---
\newcommand{\bestModel}{ConvNeXt-Tiny}
...
\newcommand{\projfig}[4]{% #1=file  #2=width(0-1)  #3=caption  #4=label
  ...
  \IfFileExists{#1}{\includegraphics[width=#2\linewidth]{#1}}{%
    ...
    \fbox{\parbox[c][0.30\textheight][c]{#2\linewidth}{\centering
      {\ttfamily\small\detokenize{#1}}\\[6pt]\itshape
...
The best-performing model, \bestModel, reached a test accuracy of $96.4\%$ and a macro-F1 of
$96.3\%$, far ahead of the from-scratch custom baseline at $47.2\%$ accuracy, confirming a decisive
...
The best model is deployed as an explainable web prototype, while the near-identical
EfficientNetV2-B0 ($96.2\%$ accuracy at $24$~MB and $9.6$~ms per image, versus $112$~MB and $113$~ms)
is identified as the most efficient deployment choice, supporting practical, trustworthy
sports-image tagging.
...
categories (sorted), showing that the dataset is approximately balanced, ranging from about 59 to 191
images per class with a mean of roughly 135.}{fig:dist}
