...
\midrule
Custom CNN          & \res{81.3} & \res{79.6} & \res{92.8} & \res{96.0} & \res{4.0}  & \res{47.8} & \res{31.5} \\
EfficientNetV2-B0   & \res{91.0} & \res{89.5} & \res{97.6} & \res{99.0} & \res{7.1}  & \res{50.1} & \res{20.4} \\
ConvNeXt-Tiny       & \res{89.1} & \res{88.0} & \res{96.5} & \res{98.4} & \res{28.6} & \res{57.0} & \res{23.2} \\
\bottomrule
\end{tabular}
\end{table}
...
ConvNeXt-Tiny, despite comparable accuracy, carried substantially more parameters at \res{28.6}~M and the largest on-disk footprint of \res{57.0}~MB, with a slightly higher latency of \res{23.2}~ms, making it the most resource-intensive option.
The custom CNN had the fewest parameters at \res{4.0}~M but, counter-intuitively, the highest inference latency of \res{31.5}~ms and a mid-range size of \res{47.8}~MB, while also delivering the lowest accuracy, so it did not occupy the efficiency frontier despite its compact parameterisation.
