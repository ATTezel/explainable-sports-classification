...
The training partition contains the remaining images and averages approximately $135$ images per class, ranging from $59$ to $191$ images per class (standard deviation $\approx21$), which provides a moderate but non-trivial amount of supervision for each category.
...
The full composition of the dataset, including its source, partition sizes, per-class counts, and image format, is summarised in Table~\ref{tab:dataset}.

\begin{table}[htbp]
\centering
\caption{Composition of the ``100 Sports Image Classification'' dataset \citep{piosenka2022sports} used in this study, summarising the official train, validation, and test partitions, the per-class image counts, the image format and resolution, and the dataset source. The validation and test partitions are exactly balanced at five images per class, whereas the training partition is near-balanced at approximately $135$ images per class.}
\label{tab:dataset}
\begin{tabular}{ll}
\toprule
...
Source / identifier   & Kaggle, \texttt{gpiosenka/sports-classification} \\
Number of classes     & $100$ sport categories \\
Total labelled images & $14{,}493$ \\
Training images       & $13{,}493$ ($59$--$191$ per class, mean ${\approx}135$) \\
Validation images     & $500$ ($5$ per class, balanced) \\
Test images           & $500$ ($5$ per class, balanced) \\
Image format          & three-channel JPG (RGB) \\
Image resolution      & $224\times224\times3$ pixels \\
Annotation            & single integer class label per image \\
...
The held-out partitions are exactly balanced at five images per class, and the training partition is near-balanced at roughly $135$ images per class (ranging from $59$ to $191$), so although not perfectly uniform no class is grossly under-represented.
...
The class-distribution plot confirms that per-class counts range from about $59$ to $191$ around a mean of approximately $135$ images, with no class exhibiting the extreme scarcity that would necessitate resampling.
...
The complete layer-by-layer structure of the custom CNN, together with the output shape produced at each stage, is summarised in Table~\ref{tab:customcnn}.

\begin{table}[htbp]
\centering
\caption{Layer-by-layer architecture of the custom CNN baseline trained from scratch. Four convolutional blocks progressively increase the channel depth from $32$ to $256$ while halving the spatial resolution at each stage, after which global average pooling and a regularised dense head produce the $100$-class softmax output. The augmentation block is active only at training time, and the output shapes assume a $224\times224\times3$ input.}
\label{tab:customcnn}
\begin{tabular}{lll}
\toprule
\textbf{Stage} & \textbf{Layer(s)} & \textbf{Output shape} \\
\midrule
Input        & Input image                                       & $224\times224\times3$ \\
Augmentation & RandomFlip/Rotation/Zoom/Contrast (train only)    & $224\times224\times3$ \\
Rescaling    & Rescaling ($1/255$)                               & $224\times224\times3$ \\
Conv block 1 & Conv2D($32$, $3{\times}3$)\,+\,BN\,+\,ReLU\,+\,MaxPool  & $112\times112\times32$ \\
Conv block 2 & Conv2D($64$, $3{\times}3$)\,+\,BN\,+\,ReLU\,+\,MaxPool  & $56\times56\times64$ \\
Conv block 3 & Conv2D($128$, $3{\times}3$)\,+\,BN\,+\,ReLU\,+\,MaxPool & $28\times28\times128$ \\
Conv block 4 & Conv2D($256$, $3{\times}3$)\,+\,BN\,+\,ReLU\,+\,MaxPool & $14\times14\times256$ \\
Pooling      & GlobalAveragePooling2D                            & $256$ \\
Dense head   & Dropout($0.4$)\,+\,Dense($256$, ReLU)\,+\,Dropout($0.3$) & $256$ \\
Output       & Dense($100$, softmax)                             & $100$ \\
\bottomrule
\end{tabular}
\end{table}
